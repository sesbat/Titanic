...
size_t QuadTreeNode::countObjects() const
{
    if(!isSubdivided)
        return std::count_if(mObjects.begin(), mObjects.end(),
                             [](auto& obj)->bool{ return obj !=nullptr; });
...
void QuadTreeNode::insert(Object* ptr)
{
    if(isSubdivided) {
        // Try to insert it in a subnode
        auto i{ getIndex(ptr->GetGlobalBound()) };

        if(contains(mChildren[i]->mBounds, ptr->GetGlobalBound())) {
...
std::vector<Object*> QuadTreeNode::getCollisionable(sf::FloatRect const& bounds) const
{
...
std::vector<Object*> QuadTreeNode::getAllObjects() const
{
    std::vector<Object*> ret;
